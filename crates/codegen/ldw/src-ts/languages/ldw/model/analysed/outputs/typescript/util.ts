    ...
    switch (type.discriminator) {
        ...
        case Model.Discriminator.NamedTypeReference:
            if (type.fqn.length === 1) {
                ...
                const namespace = pascalCase(type.fqn.slice(0, -1).join('_'));
                return `${namespace}.${pascalCase(type.fqn[type.fqn.length - 1])}`;
            }
    }
}

export function typeAsTypescriptDescription(type: Model.Type): string {
    switch (type.discriminator) {
        case Model.Discriminator.VoidType:
            return 'Void';
        case Model.Discriminator.PrimitiveType:
        ...
        case Model.Discriminator.MapType:
            return `${typeAsTypescriptDescription(type.keyType)}To${typeAsTypescriptDescription(type.valueType)}Map`;
        case Model.Discriminator.SetType:
            return `${typeAsTypescriptDescription(type.keyType)}Set`;
        case Model.Discriminator.SequenceType:
            return `${typeAsTypescriptDescription(type.elementType)}Array`;
        case Model.Discriminator.OptionType:
            return `${typeAsTypescriptDescription(type.type)}OrUndefined`;
        case Model.Discriminator.NamedTypeReference:
