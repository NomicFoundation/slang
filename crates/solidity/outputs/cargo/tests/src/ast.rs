...
use slang_solidity::{ast, parser::Parser, utils::LanguageFacts};

#[test]
fn test_build_source_unit_ast() -> Result<()> {
    let parser = Parser::create(LanguageFacts::LATEST_VERSION)?;
    let output = parser.parse_file_contents(r###"
...
contract MyContract {
    ...
    let ast = ast::builder::build_source_unit(output.create_tree_cursor()).map_err(|s| anyhow!(s))?;
    assert_eq!(2, ast.members.len());
    assert!(matches!(ast.members[0], ast::SourceUnitMember::PragmaDirective(_)));
    assert!(matches!(ast.members[1], ast::SourceUnitMember::ContractDefinition(_)));
    ...
    assert!(matches!(state_var.type_name, ast::TypeName::ElementaryType(_)));
    ...
    assert_eq!(1, function.returns.as_ref().unwrap().variables.parameters.len());
