...
impl ContractDefinitionStruct {
    ...
    fn lay_out_state_variables<'a>(
        ...
        for state_variable in variables {
            ...
            let variable_type_id = self.semantic.binder.try_node_typing(node_id)?.as_type_id()?;
...
impl ParametersStruct {
    pub(crate) fn compute_abi_parameters(&self) -> Option<Vec<AbiParameter>> {
        ...
        for parameter in &self.ir_nodes {
            ...
            let type_id = self.semantic.binder.try_node_typing(node_id)?.as_type_id()?;
    ...
    fn type_as_abi_parameter(&self, type_id: TypeId) -> Option<(String, Vec<ParameterComponent>)> {
        match self.types.get_type_by_id(type_id) {
            ...
            Type::Struct { definition_id, .. } => {
                ...
                let Definition::Struct(definition) =
