...
;; *also* needs to (optionally) propagate the dynamic scope to be able to
;; correctly bind `using` attached functions.
@source_unit [SourceUnit [SourceUnitMembers
    [SourceUnitMember @contract_or_library ([ContractDefinition] | [LibraryDefinition])]
]] {
  edge @source_unit.lexical_scope -> @contract_or_library.def
  edge @source_unit.defs -> @contract_or_library.def

  ; When "leaving" the contract lexical scope we need to (optionally) propagate
  ; the dynamic scope (ie. `using` directives extensions)
  node @contract_or_library.dynamic
  attr (@contract_or_library.dynamic) is_exported

  node @contract_or_library.push_dynamic
  ...
  edge @contract_or_library.lexical_scope -> @contract_or_library.push_dynamic
  edge @contract_or_library.push_dynamic -> push
  edge push -> pop
  edge pop -> @source_unit.lexical_scope
  edge @contract_or_library.push_dynamic -> @source_unit.lexical_scope
  ...
  let @contract_or_library.parent_scope = @contract_or_library.push_dynamic
}
...
@specifier [InheritanceSpecifier [InheritanceTypes
  ...
  ; and from the heir's internal scope (so that inherited internal variables can
  ; be accessed)
  node internal
...
@contract [ContractDefinition @name name: [Identifier]] {
  ...
  ; From a call we may need to resolve using the dynamic scope, in case there's
...
@library [LibraryDefinition @name name: [Identifier]] {
  ...
  ; Provide a path to the parent lexical context that pushes our own name to the
  ; symbol stack. This allows a local symbol (eg. a struct) defined and resolved
  ; in this library to be fully qualified in order for `using` directives that
  ; attach our own functions to our own types to be used (see
  ; bindings_output/using/fqn_library test).
  ; TODO: we should probably apply this same pattern to aliases from import
  ; statements.
  node push_member
  attr (push_member) push_symbol = "."
  node push_name
  attr (push_name) push_symbol = (source-text @name)
  edge @library.lexical_scope -> push_member
  edge push_member -> push_name
  edge push_name -> @library.parent_scope
}
