...
contract YaxisVotePower is IVoteProxy {
  ...
  constructor(address _yax, address _yaxisChef, address _yaxisBar, address _yaxEthUniswapV2Pair) public {
    yax = IERC20(_yax);
  ...
  function totalSupply() external view override returns (uint256 _supply) {
    ...
    uint256 _lpStakingYax = _yaxReserves.mul(yaxEthUniswapV2Pair.balanceOf(address(chef))).div(_supply);
    _supply = sqrt(yax.totalSupply().add(_lpStakingYax).add(yaxisBar.availableBalance()));
  }

  function balanceOf(address _voter) external view override returns (uint256 _balance) {
    (uint256 _stakeAmount, , ) = chef.userInfo(PID, _voter);
    (uint256 _yaxReserves, , ) = yaxEthUniswapV2Pair.getReserves();
    uint256 _supply = yaxEthUniswapV2Pair.totalSupply();
    _supply = _supply == 0 ? 1e18 : _supply;
    uint256 _lpStakingYax = _yaxReserves.mul(_stakeAmount).div(_supply).add(chef.pendingYaxis(PID, _voter));
    _supply = yaxisBar.totalSupply();
    _supply = _supply == 0 ? 1e18 : _supply;
    uint256 _syaxAmount = yaxisBar.balanceOf(_voter).mul(yaxisBar.availableBalance()).div(_supply);
    _balance = sqrt(yax.balanceOf(_voter).add(_lpStakingYax).add(_syaxAmount));
  }
 ...
 * this contract only.
 */
contract yAxisMetaVaultManager is
  IVaultManager // solhint-disable-line contract-name-camelcase
{
 ...
 */
contract yAxisMetaVaultHarvester is
  IHarvester // solhint-disable-line contract-name-camelcase
{
  using SafeMath for uint256;
   ...
   */
  event Harvest(address indexed controller, address indexed strategy);
   ...
   */
  function addStrategy(address _token, address _strategy, uint256 _timeout) external override onlyStrategist {
   ...
   * @param _timeout The timeout between harvests
   */
  function removeStrategy(address _token, address _strategy, uint256 _timeout) external override onlyStrategist {
   ...
   * @param _strategy The address of the strategy
   */
  function harvest(IController _controller, address _strategy) public onlyHarvester {
...
contract StrategyControllerV2 is IController {
   ...
   */
  event StrategiesReordered(address indexed token, address indexed strategy1, address indexed strategy2);
  ...
  function inCaseStrategyGetStuck(address _strategy, address _token) external onlyStrategist {
    ...
    IERC20(_token).safeTransfer(vaultManager.governance(), IERC20(_token).balanceOf(address(this)));
  }
   ...
   * @param _timeout The timeout between harvests
   */
  function removeStrategy(address _token, address _strategy, uint256 _timeout) external onlyStrategist {
   ...
   */
  function reorderStrategies(address _token, address _strategy1, address _strategy2) external onlyStrategist {
    require(_strategy1 != _strategy2, "_strategy1 == _strategy2");
    TokenStrategy storage tokenStrategy = tokenStrategies[_token];
    // ensure the strategies are already added
    require(tokenStrategy.active[_strategy1] && tokenStrategy.active[_strategy2], "!active");
   ...
   */
  function setCap(address _token, address _strategy, uint256 _cap) external onlyStrategist {
    require(tokenStrategies[_token].active[_strategy], "!active");
    tokenStrategies[_token].caps[_strategy] = _cap;
   ...
   */
  function setConverter(address _input, address _output, address _converter) external onlyStrategist {
  ...
  function earn(address _token, uint256 _amount) external override onlyVault(_token) {
    ...
    if (_want != _token) {
      ...
      _amount = IConverter(_converter).convert(_token, _want, _amount);
  ...
  function withdraw(address _token, uint256 _amount) external override onlyVault(_token) {
    (address[] memory _strategies, uint256[] memory _amounts) = getBestStrategyWithdraw(_token, _amount);
  ...
  function balanceOf(address _token) external view override returns (uint256 _balance) {
    ...
    for (uint i = 0; i < k; i++) {
      ...
      if (_want != _token) {
        address _converter = converters[_token][_want];
        _balance = _balance.add(IConverter(_converter).convert_rate(_want, _token, _strategy.balanceOf()));
      } else {
   ...
   * @param _amount The amount that will be withdrawn
   */
  function withdrawFee(address, uint256 _amount) external view override returns (uint256 _fee) {
   ...
   * @param _amount The amount that will be invested
   */
  function getBestStrategyEarn(address _token, uint256 _amount) public view returns (address _strategy) {
    // get the index of the last strategy
    uint256 k = tokenStrategies[_token].strategies.length;
  ...
  function getBestStrategyWithdraw(
    address _token,
    uint256 _amount
  ) public view returns (address[] memory _strategies, uint256[] memory _amounts) {
  ...
  modifier onlyStrategist() {
    require(msg.sender == vaultManager.strategist() || msg.sender == vaultManager.governance(), "!strategist");
    _;
  }
...
abstract contract BaseStrategy is IStrategy {
  ...
  function _payHarvestFees(address _poolToken) internal returns (uint256 _wethBal) {
  ...
  modifier onlyAuthorized() {
    require(
      msg.sender == controller || msg.sender == vaultManager.strategist() || msg.sender == vaultManager.governance(),
      "!authorized"
    );
    _;
  }
...
contract StrategyYearnV2 is BaseStrategy {
  ...
  constructor(
    ...
    address _weth,
    address _router
  ) public BaseStrategy(_name, _controller, _vaultManager, _underlying, _weth, _router) {
  ...
  function balanceOfPool() public view override returns (uint256) {
    ...
    return balance.mul(IYearnV2Vault(yvToken).pricePerShare()).div(1e18);
  }
...
contract yAxisMetaVault is ERC20, IMetaVault {
  ...
  constructor(
    ...
    uint _yaxPerBlock,
    uint _startBlock
  ) public ERC20("yAxis.io:MetaVault:3CRV", "MVLT") {
      ...
      if (_to >= epochEndBlocks[epochId - 1]) {
        ...
        for (epochId = epochId - 1; epochId >= 1; --epochId) {
          ...
          if (_from >= epochEndBlocks[epochId - 1])
            return multiplier.add(epochEndBlocks[epochId].sub(_from).mul(epochRewardMultiplers[epochId]));
          // accumulate the multipler with the reward from the epoch
          multiplier = multiplier.add(
            epochEndBlocks[epochId].sub(epochEndBlocks[epochId - 1]).mul(epochRewardMultiplers[epochId])
          );
        }
   ...
   */
  function available() public view override returns (uint) {
   ...
   */
  function withdrawFee(uint _amount) public view override returns (uint) {
   ...
   */
  function calc_token_amount_deposit(uint[3] calldata amounts) external view override returns (uint) {
   ...
   */
  function calc_token_amount_withdraw(uint _shares, address _output) external view override returns (uint) {
   ...
   */
  function convert_rate(address _input, uint _amount) external view override returns (uint) {
   ...
   */
  function getPricePerFullShare() external view override returns (uint) {
...
contract StrategyGenericVault is BaseStrategy {
  ...
  constructor(
    ...
    address _weth,
    address _router
  ) public BaseStrategy(_name, _controller, _vaultManager, IGenericVault(_vault).token(), _weth, _router) {
  ...
  function balanceOfPool() public view virtual override returns (uint256) {
    ...
    return IERC20(vault).balanceOf(address(this)).mul(getPricePerFullShare()).div(1e18);
  }
...
contract StrategyPickle3Crv is BaseStrategy {
  ...
  constructor(
    ...
    address _vaultManager,
    address _router
  ) public BaseStrategy(_name, _controller, _vaultManager, _want, _weth, _router) {
  ...
  function setStableForLiquidity(address _stableForAddLiquidity) external onlyAuthorized {
    require(
      _stableForAddLiquidity == dai || _stableForAddLiquidity == usdc || _stableForAddLiquidity == usdt,
...
contract StableSwap3PoolConverter is IConverter {
   ...
   * @param _amount The amount to spend
   */
  function approveForSpender(IERC20 _token, address _spender, uint256 _amount) external onlyGovernance {
    _token.safeApprove(_spender, _amount);
  }
  ...
  function calc_token_amount(
    uint256[3] calldata amounts,
    bool deposit
  ) external view override returns (uint256 _shareAmount) {
   ...
   */
  function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyGovernance {
    _token.safeTransfer(_to, _amount);
  }
   ...
   */
  modifier onlyGovernance() {
    require(vaultManager.controllers(msg.sender) || msg.sender == vaultManager.governance(), "!governance");
    _;
  }
...
contract StableSwap3PoolOracle is IStableSwap3PoolOracle {
  ...
  constructor(address _feedETHUSD, address _feedDAIETH, address _feedUSDCETH, address _feedUSDTETH) public {
    ethUsd = _feedETHUSD;
    feeds[0] = _feedDAIETH;
...
contract StrategydYdXSoloMargin is BaseStrategy {
  ...
  constructor(
    ...
    BaseStrategy(
      string(
        abi.encodePacked("dYdX SoloMargin: ", ERC20(ISoloMargin(_dYdX).getMarketTokenAddress(_marketId)).symbol())
      ),
      _controller,
      _vaultManager,
...
contract MockdYdXSoloMargin is ISoloMargin {
  ...
  function operate(Account.Info[] memory accounts, Actions.ActionArgs[] memory actions) public override {
    ...
    _runActions(accounts, actions);
  }

  function _verifyInputs(Account.Info[] memory accounts, Actions.ActionArgs[] memory actions) private pure {
  ...
  function _runActions(Account.Info[] memory accounts, Actions.ActionArgs[] memory actions) private {
  ...
  function _deposit(Actions.DepositArgs memory args) private {
    require(args.from == msg.sender || args.from == args.account.owner, "Invalid deposit source");
  ...
  function getMarketTokenAddress(uint256 marketId) external view override returns (address) {
  ...
  function getAccountWei(
    ...
    uint256 marketId
  ) external view override returns (Types.Wei memory) {
    Types.Wei memory balance = Types.Wei({ sign: true, value: balances[account.owner][marketId] });
    return balance;
  }
...
contract MockzpaToken is ERC20, IZPAToken {
  ...
  constructor(string memory _name, string memory _symbol, address _underlyingAsset) public ERC20(_name, _symbol) {
  ...
  function redeem(uint256 _amount) external override {
    ...
    uint256 feeSubtraction = initialFee.sub(endFee).mul(block.timestamp.sub(userInfo[_msgSender()].depositTime)).div(
      feeDuration
    );
...
contract StrategyStabilize is BaseStrategy {
  ...
  function balanceOfPool() public view override returns (uint256) {
    ...
    uint256 zpaBalance = balanceOfzpaToken().mul(_zpaToken.pricePerToken()).div(1e18);
    return
      (IZPAPool(pool).poolBalance(poolId, address(this))).mul(_zpaToken.pricePerToken()).div(1e18).add(zpaBalance).sub(
        calculateZPATokenWithdrawFee(zpaBalance)
      );
  }
...
contract MockStabilizePool is IZPAPool {
  ...
  constructor(address _lpToken, address _rewardToken, uint256 _rewardRate) public {
...
contract StrategyIdle is BaseStrategy {
  ...
  function balanceOfPool() public view override returns (uint256) {
    ...
    return balance.mul(pricePerToken()).div(1e18);
  }
...
contract MockCurveGauge is Gauge {
  ...
  function claimable_tokens(address _account) external view override returns (uint) {
...
contract MockYaxisBar is ERC20 {
  ...
  constructor(address _yax) public ERC20("Staked yAxis", "sYAX") {
    YAX = IERC20(_yax);
  }
  ...
  function getPricePerFullShare() public view returns (uint256) {
    return totalSupply() == 0 ? 1e18 : YAX.balanceOf(address(this)).mul(1e18).div(totalSupply());
  }
}
...
contract MockUniswapPair is ERC20 {
  ...
  constructor(address _token0, address _token1) public ERC20("Uniswap Pair", "UNI-V2") {
    token0 = _token0;
    token1 = _token1;
  }

  function addLiquidity(uint256 _amount0, uint256 _amount1, uint256 _amountOut) external {
...
contract StrategyDforce is BaseStrategy {
  ...
  function balanceOfPool() public view override returns (uint256) {
    return
      (dRewards(pool).balanceOf(address(this))).mul(dERC20(dToken).getExchangeRate()).div(1e18).add(balanceOfdToken());
  }
