...
test("binding graph", async () => {
  ...
  assert.strictEqual(cursor.node.unparse(), "Parent");
  ...
  //          ^^^^^
  assert(cursor.goToNextTerminalWithKind(TerminalKind.Identifier));
  assert.strictEqual(cursor.node.unparse(), "Child");
  ...
  {
    assert.strictEqual(unit.bindingGraph.referenceAt(cursor), undefined);
  }
  ...
  {
    assert.strictEqual(unit.bindingGraph.definitionAt(cursor), undefined);
  }
  ...
  //            ^^^
  assert(cursor.goToNextTerminalWithKind(TerminalKind.Identifier));
  assert.strictEqual(cursor.node.unparse(), "foo");
  ...
  //     ^^^^^^
  assert(cursor.goToNextTerminalWithKind(TerminalKind.Identifier));
  assert.strictEqual(cursor.node.unparse(), "assert");
