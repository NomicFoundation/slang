...
use sourcify::{Buffers, Repository};
...
fn run_test_command(cmd: &command::TestCommand) -> Result<()> {
    ...
    for shard in shards {
        ...
        let mut contract_iter = archive.contract_iter();

        while let Some(c) = contract_iter.next_contract(&mut buffers) {
            match c {
                Ok(contract) => {
                    events.trace(format!("Starting {}", contract.name));

                    let sources_count = contract.sources_count();
                    events.inc_files_count(sources_count);

                    let mut did_fail = false;
                    match contract.create_compilation_unit(&mut buffers) {
                        Ok(unit) => {
                ...
                Err(e) => {
                    events.trace(format!("Failed to get contract: {e}"));
                }
            }
        }
