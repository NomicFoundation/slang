...
The Solidity projects and files tested in the above benchmarks are declared in [`config.rs`](../utils/src/config.rs). The source for these projects correspond to contracts in Ethereum Mainnet, and they are downloaded from Sourcify (see [`fetch.rs`](../utils/src/fetch.rs)). It's split in two sections:

- `PROJECTS`: full Solidity projects. The intention is to test the main entry point as defined by the project, and follow from there all of the dependencies.
- `FILES`: specific files to be tested, that are intended to be self-contained. Each file might or might not be the entrypoint of the Solidity project referred by the hash; for that reason, the entrypoint must be specified via the `file` field.
