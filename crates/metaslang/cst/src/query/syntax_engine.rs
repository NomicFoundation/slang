...
use crate::syntax_cursor::SyntaxCursor;

impl<T: KindTypes + 'static> SyntaxCursor<T> {
    /// Returns an iterator over all matches of the given queries in the syntax tree.
    pub fn query(self, queries: Vec<Query<T>>) -> SyntaxQueryMatchIterator<T> {
        SyntaxQueryMatchIterator::new(self, queries)
    }
    ...
    fn matches_node_selector(&self, node_selector: &NodeSelector<T>) -> bool {
        if let Some(nonterminal) = self.node().as_nonterminal() {
            match node_selector {
                ...
                NodeSelector::EdgeLabel { edge_label } => *edge_label == self.node().label(),
                NodeSelector::EdgeLabelAndNodeKind {
                    edge_label,
                    node_kind,
                } => {
                    *edge_label == self.node().label()
                        && NodeKind::Nonterminal(nonterminal.kind) == *node_kind
                }
                NodeSelector::EdgeLabelAndNodeText { .. } => false,
            }
        } else if let Some(terminal) = self.node().as_terminal() {
            if terminal.kind.is_trivia() {
                false
            } else {
                match node_selector {
                    NodeSelector::Anonymous => true,
                    ...
                    NodeSelector::EdgeLabelAndNodeText {
                        edge_label,
                        node_text,
                    } => *edge_label == self.node().label() && terminal.text == *node_text,
                }
            }
        } else {
            unreachable!("node is neither terminal nor non-terminal");
        }
    }
}
...
impl<T: KindTypes + 'static> ASTNode<T> {
    ...
    fn can_match_syntax(&self, cursor: &SyntaxCursor<T>) -> bool {
        match self {
...
pub struct SyntaxQueryMatch<T: KindTypes> {
    queries: Rc<Vec<Query<T>>>,
    query_index: usize,
    root_cursor: SyntaxCursor<T>,
    captures: BTreeMap<String, Vec<SyntaxCursor<T>>>,
}

impl<T: KindTypes> SyntaxQueryMatch<T> {
    ...
    pub fn root_cursor(&self) -> &SyntaxCursor<T> {
        &self.root_cursor
    }
    ...
    pub fn capture(&self, name: &str) -> Option<SyntaxCapture<'_, T>> {
        ...
        Some(SyntaxCapture { name, cursors })
    }
}
...
impl<T: KindTypes> SyntaxCapture<'_, T> {
    ...
    pub fn cursors(&self) -> &[SyntaxCursor<T>] {
        self.cursors
    }
}

/// Iterator over query matches in the syntax tree.
pub struct SyntaxQueryMatchIterator<T: KindTypes> {
    queries: Rc<Vec<Query<T>>>,
    cursor: SyntaxCursor<T>,
    query_index: usize,
...
impl<T: KindTypes + 'static> SyntaxQueryMatchIterator<T> {
    fn new(cursor: SyntaxCursor<T>, queries: Vec<Query<T>>) -> Self {
        Self {
    ...
    fn advance_to_next_possible_matching_query(&mut self) {
        while !self.cursor.is_completed() {
            while self.query_index < self.queries.len() {
                ...
                if ast_node.can_match_syntax(&self.cursor) {
                    // The first matcher in the query should allow implicit matches
                    self.matcher = Some(ast_node.create_syntax_matcher(self.cursor.clone(), false));
                    return;
                }
                self.query_index += 1;
            }
...
impl<T: KindTypes + 'static> Iterator for SyntaxQueryMatchIterator<T> {
    type Item = SyntaxQueryMatch<T>;
...
trait Matcher<T: KindTypes> {
    ...
    fn record_captures(&self, captures: &mut BTreeMap<String, Vec<SyntaxCursor<T>>>);
}
...
impl<T: KindTypes + 'static> CaptureMatcher<T> {
    fn new(
        ...
        let child = matcher
            .child
            .create_syntax_matcher(cursor.clone(), require_explicit_match);
        Self {
            matcher,
            cursor,
            child,
...
impl<T: KindTypes> NodeMatchMatcher<T> {
    fn new(matcher: Rc<NodeMatchASTNode<T>>, cursor: SyntaxCursor<T>) -> Self {
        Self {
            matcher,
            child: None,
            cursor,
    ...
    fn next(&mut self) -> Option<MatcherResult<T>> {
        ...
        if !self.is_initialised {
            ...
            if let Some(child) = self.matcher.child.as_ref() {
                ...
                self.child = Some(child.create_syntax_matcher(child_cursor, false));
            } else {
    ...
    fn create_syntax_matcher(
        ...
        match item {
            ...
            SequenceItem::ChildMatcher(index) => {
                self.matcher.children[*index].create_syntax_matcher(cursor, require_explicit_match)
            }
        }
    }
}
...
impl<T: KindTypes + 'static> Matcher<T> for SequenceMatcher<T> {
    fn next(&mut self) -> Option<MatcherResult<T>> {
        if !self.is_initialised {
            ...
            let child = self.create_syntax_matcher(0, child_cursor, self.require_explicit_match);
        ...
        while !self.children.is_empty() {
            if let Some(child_matcher_result) = self.children.last_mut().unwrap().next() {
                ...
                let child = self.create_syntax_matcher(
                    self.children.len(),
    ...
    fn next(&mut self) -> Option<MatcherResult<T>> {
        if let Some(child) = self.child.as_mut() {
            ...
            let child = self
                .matcher
                .child
                .create_syntax_matcher(child_cursor, self.require_explicit_match);
            self.child = Some(child);
        ...
        loop {
            if let Some(last_result) = self.result_for_next_repetition.take() {
                let next_child = self
                    .matcher
                    .child
                    .create_syntax_matcher(last_result.cursor, last_result.require_explicit_match);
...
impl<T: KindTypes> EllipsisMatcher<T> {
    fn new(cursor: SyntaxCursor<T>, require_explicit_match: bool) -> Self {
        Self {
            cursor,
...
impl<T: KindTypes + 'static> Matcher<T> for EllipsisMatcher<T> {
    ...
    fn record_captures(&self, _: &mut BTreeMap<String, Vec<SyntaxCursor<T>>>) {}
}

/// Greedily consumes available trivia nodes only
struct AdjacencyMatcher<T: KindTypes> {
    cursor: Option<SyntaxCursor<T>>,
    require_explicit_match: bool,
}
