...
impl<T: KindTypes> SyntaxCursor<T> {
    ...
    pub fn descendants(&self) -> SyntaxCursorIterator<T> {
        let mut cursor = self.spawn();

        // skip the current node:
        cursor.go_to_next();

        SyntaxCursorIterator { cursor }
    }
    ...
    pub fn remaining_nodes(&self) -> SyntaxCursorIterator<T> {
        SyntaxCursorIterator {
            cursor: self.clone(),
        }
    }
    ...
    pub fn ancestors(&self) -> SyntaxAncestorsIterator<T> {
        let current = Rc::clone(&self.node);

        SyntaxAncestorsIterator { current }
    }
...
pub struct SyntaxCursorIterator<T: KindTypes> {
    cursor: SyntaxCursor<T>,
}

impl<T: KindTypes> Iterator for SyntaxCursorIterator<T> {
...
pub struct SyntaxAncestorsIterator<T: KindTypes> {
    current: Rc<SyntaxNode<T>>,
}

impl<T: KindTypes> Iterator for SyntaxAncestorsIterator<T> {
    type Item = Rc<SyntaxNode<T>>;
