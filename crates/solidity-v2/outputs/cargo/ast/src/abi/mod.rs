...
impl ContractAbi {
    ...
    /// The first storage slot left free after the persistent (mutable) layout.
    /// A region appended at this slot cannot alias the tail of a multi-slot
    /// variable, so a consumer can lay out further state starting here.
...
pub(crate) fn extract_function_type_parameters_abi(
    ...
    // A multi-value getter (e.g. a public struct's accessor) has a tuple return
    // type, expanded into one ABI parameter per element; a scalar return is one.
    let output_types = match semantic.types().get_type_by_id(function_type.return_type) {
        Type::Tuple(TupleType { types }) => types.clone(),
        _ => vec![function_type.return_type],
    };
