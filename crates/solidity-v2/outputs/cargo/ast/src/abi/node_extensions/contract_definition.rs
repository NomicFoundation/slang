...
impl ContractDefinitionStruct {
    ...
    /// Computes the storage layouts of the permanent, transient, and immutable
    /// state variables.
    ///
    /// `immutable` variables have no native storage — they are encoded into the
    /// deployed code. The experimental solx backend instead lowers them as
    /// storage, appended after the persistent layout, so that a read following
    /// the constructor's write observes the assigned value. They therefore begin
    /// at the first slot left free by the persistent layout.
    fn compute_storage_layout(
        ...
        let (immutable_storage_layout, _) = self.lay_out_state_variables(
            next_free_slot,
            all_state_variables.iter().filter(|state_variable| {
                matches!(
                    state_variable.mutability(),
