...
impl Expression {
    ...
    /// Peels redundant parentheses to a fixpoint. The grammar models a
    /// parenthesised expression as a single-element tuple, so `(x)` and `((x))`
    /// read as the bare inner expression `x`; returns `self` if not parenthesised.
    #[must_use]
    ...
    /// constant of integer type, or `None` otherwise. The typing pass records
    /// the value on the expression's `Literal` type (so `1 + 2` carries it too).
    pub fn integer_value(&self) -> Option<BigInt> {
        let Type::Literal(literal_type) = self.get_type()? else {
            return None;
        };
        match Number::from_literal_kind(&literal_type.kind())? {
