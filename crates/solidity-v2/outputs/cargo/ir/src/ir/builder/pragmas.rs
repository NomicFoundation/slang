...
use slang_solidity_v2_common::diagnostics::kinds::structure::DuplicateExperimentalFeature;
...
impl<S: Source> CstToIrBuilder<'_, S> {
    pub(super) fn build_experimental_pragma(
        ...
        if feature != output::ExperimentalFeature::Unrecognized
            && !self.experimental_features.insert(feature)
        {
            self.report(&source.feature, DuplicateExperimentalFeature);
        }
