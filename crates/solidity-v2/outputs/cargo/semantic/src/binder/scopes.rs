...
impl Scope {
    ...
    pub(crate) fn insert_definition(&mut self, symbol: StringId, node_id: NodeId) {
        match self {
            ...
            Self::Function(function_scope) => function_scope.insert_definition(symbol, node_id),
            Self::Modifier(modifier_scope) => modifier_scope.insert_definition(symbol, node_id),
            Self::Parameters(_) => {
                unreachable!("cannot insert a definition into parameters scope directly")
            }
            Self::Struct(struct_scope) => struct_scope.insert_definition(symbol, node_id),
            Self::Using(_) => unreachable!("cannot insert a definition into a using clause scope"),
            Self::YulBlock(yul_block_scope) => yul_block_scope.insert_definition(symbol, node_id),
            Self::YulFunction(function_scope) => function_scope.insert_definition(symbol, node_id),
        }
    }
...
impl ParametersScope {
    ...
    pub(crate) fn add_parameter(&mut self, identifier: Option<StringId>, node_id: NodeId) {
    ...
    pub(crate) fn lookup_definition(&self, symbol: StringId) -> Option<NodeId> {
        self.parameters
            .iter()
            .find(|parameter| parameter.name == Some(symbol))
