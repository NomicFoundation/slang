...
/// The same when both candidates are library functions and the receiver is a
/// contract value. A `using` directive does not make either one a member of
/// that contract, so they stay competing candidates and the call is reported.
#[test]
fn test_attached_library_functions_on_a_contract_value_stay_ambiguous() {
    const SOURCE: &str = r###"
pragma solidity *;
...
contract A {
    ...
    function test(B b) internal {
        b.foo();
    }
}
    "###;

    // The call is only reported because both candidates survive the override
    // filter. Dropping either one would resolve it to the other.
    let analysis = Analysis::of_source(SOURCE).run(Analyse::References);
    assert_eq!(
        Some(
            AmbiguousReference {
