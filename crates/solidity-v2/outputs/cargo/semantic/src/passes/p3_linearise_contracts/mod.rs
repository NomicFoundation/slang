...
pub fn run(binder: &mut Binder, files: &[File]) {
    ...
    for file in files {
        pass.visit_file_collect_bases(file.source_unit());
    }
    for file in files {
        pass.visit_file_linearise_contracts(file.source_unit());
    }
}
...
impl<'a> Pass<'a> {
    ...
    fn resolve_identifier_path_in_scope(
        ...
        for element in identifier_path {
            let identifier = match element {
                ir::IdentifierPathElement::Identifier(identifier) => identifier,
                ir::IdentifierPathElement::AddressKeyword => {
                    // address can't be a valid base contract, so resolution fails
