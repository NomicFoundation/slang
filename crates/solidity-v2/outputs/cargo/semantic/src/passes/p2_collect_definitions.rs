...
/// passes.
pub fn run(binder: &mut Binder, files: &[File]) {
...
impl<'a> Pass<'a> {
    ...
    fn current_file(&self) -> &'a File {
        self.current_file.expect("no current file set")
    }
...
impl Visitor for Pass<'_> {
        ...
        match node.kind {
            ...
            | ir::FunctionKind::Unnamed => {
                ...
                if node.name.is_some() {
                    let visibility = node.visibility.clone();
                    let definition =
    ...
    fn leave_variable_declaration_statement(&mut self, node: &ir::VariableDeclarationStatement) {
        // In Solidity >= 0.5.0 (always true in v2) this definition should only
        // be available for statements after this one. So we open a new scope
        // that replaces but is linked to the current one
            ...
            ir::VariableDeclarationTarget::MultiTypedDeclaration(multi) => {
                for element in &multi.elements {
                    if let Some(declaration) = &element.member {
                        let definition = Definition::new_variable(declaration);
                        self.insert_definition_in_current_scope(definition);
                    }
                }
            }
    ...
    fn enter_for_statement(&mut self, node: &ir::ForStatement) -> bool {
        // Open a new block here to hold declarations in the initialization clause.
        let scope = Scope::new_block(node.id(), self.current_scope_id());
        self.enter_scope(scope);
        true
    }
