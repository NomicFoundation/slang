...
/// there overrides it, as for an interface member the contract leaves
/// unimplemented. p4 has already dropped every function a more-derived one
/// overrides, so at most one of `functions` can match.
///
