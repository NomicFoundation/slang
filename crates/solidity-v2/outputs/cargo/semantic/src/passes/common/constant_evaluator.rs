...
pub(crate) enum ComptimeResolution<Scope> {
    ...
    Constant {
        value: ir::Expression,
        target_scope: Scope,
        /// The `TypeId` of the constant's declared type, or `None` if the type
        /// is not yet known (e.g. due to a forward reference).
...
impl<Scope> CompileConstantEvaluator<'_, Scope> {
    ...
    fn evaluate_identifier(
        ...
        // Reject unresolved types (e.g. due to a forward reference).
        let type_id = type_id.ok_or(EvaluationError::Other)?;

        // Reject non-integer types.
        let Type::Integer(integer_type) = self.types.get_type_by_id(type_id) else {
            return Err(EvaluationError::Other);
        };
        let integer_type = integer_type.clone();
                    ...
                    .expect("resolved definition is found")
                {
                    Definition::Constant(constant_definition) => {
                        if let Some(value) = &constant_definition.ir_node.value {
                            // The constant's declared type id (or `None` for a
                            // not-yet-typed forward reference).
                            let type_id = self
                                .binder
                                .node_typing(constant_definition.ir_node.id())
                                .as_type_id();
...
mod tests {
    ...
    struct MapResolver {
        ...
        // The tests model every known constant as a `uint256`; this is its
        // `TypeId` in the registry the evaluator is given.
        ...
        fn resolve_identifier_in_scope(
                ...
                self.context.get(&format!("{scope}{identifier}"))
            {
                ComptimeResolution::Constant {
                    ...
                    type_id: Some(self.constant_type_id),
                }
    ...
    impl MapResolver {
        ...
        fn with_context(context: &[(&str, &str, &str)], constant_type_id: TypeId) -> Self {
            Self {
        ...
        fn recognise_erc7201_with_context(
            context: &[(&str, &str, &str)],
            constant_type_id: TypeId,
        ) -> Self {
            Self {
    ...
    fn eval_string_with_context(input: &str, context: &[(&str, &str, &str)]) -> Option<Number> {
        ...
        let resolver = MapResolver::with_context(context, types.uint256());
    ...
    fn eval_string_with_erc7201_and_context(
        ...
        let mut types = TypeRegistry::new(LanguageVersion::LATEST);
        let resolver = MapResolver::recognise_erc7201_with_context(context, types.uint256());
        let expression = parse_expression(input);
        evaluate_compile_time_number_constant(&expression, String::new(), &mut types, &resolver)
            .ok()
    }
