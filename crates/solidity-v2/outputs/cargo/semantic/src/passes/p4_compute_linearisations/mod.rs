...
impl<'a> HierarchyChecker<'a> {
    ...
    fn check(
        ...
        // TODO: each check walks the hierarchy on its own, so a type is visited
        // several times. Build one index of the members per type instead, and
        // let the checks read it. Reference resolution in a contract scope
        // could read it too.
    ...
    fn fold_base(&mut self, base_id: NodeId) {
        ...
        if self.contract.is_some_and(|contract| !contract.is_abstract) {
            ...
            self.record_abstract(members, in_interface);
        }
