...
use crate::overrides::{GetterSlot, Overrides};
...
pub(super) fn compute_linearisations(
    ...
    ContractLinearisations {
        functions: linearise_functions(&Overrides::new(binder, types), &contract_base_members),
        state_variables,
        errors,
        events,
    }
            ...
            match member {
                    ...
                    ) =>
                {
                    let already_overridden = functions
                        .iter()
                        .any(|kept| overrides.function_overrides(kept, function))
                        || getters
                            .iter()
                            .any(|getter| overrides.getter_overrides(getter, function));
                ...
                ir::ContractMember::StateVariableDefinition(state_variable) => {
                    ...
                    getters.extend(overrides.getter_slot(state_variable));
                }
                _ => {}
            }
        }
