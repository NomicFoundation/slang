...
lalrpop_mod!(
    ...
    // The `&mut DiagnosticCollection` grammar parameter introduces an unused lifetime
    // in some generated parser functions:
...
impl Parser {
    pub fn parse(file_id: &str, source: &str, language_version: LanguageVersion) -> ParseOutput {
        ...
        let result = parser.parse(source, file_id, &mut diagnostics, lexer);
        match result {
            Ok(source_unit) => {
